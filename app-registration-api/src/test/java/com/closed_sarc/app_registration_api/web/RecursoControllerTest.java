...
class RecursoControllerTest {
  ...
  void deveRetornar201EcriarRecursoQuandoDadosValidos() throws Exception {
    // Given
    RecursoRequestDTO request = RecursoRequestDTO.builder()
        .nome("Sala 401")
        .tipo("Sala de Aula")
        .quantidade(1)
        .capacidade(40)
    ...
    RecursoDTO recursoCriado = RecursoDTO.builder()
        .id(UUID.randomUUID())
        .nome("Sala 401")
        .tipo("Sala de Aula")
        .quantidade(1)
    ...
    when(recursoService.create(org.mockito.ArgumentMatchers.any(RecursoRequestDTO.class)))
  ...
  void deveRetornar400QuandoRecursoJaExiste() throws Exception {
    ...
    when(recursoService.create(org.mockito.ArgumentMatchers.any(RecursoRequestDTO.class)))
        .thenThrow(new RuntimeException("Já existe um recurso ativo com este nome"));
  ...
  void deveRetornar200EListaRecursos() throws Exception {
    // Given
    RecursoDTO recurso1 = RecursoDTO.builder()
        .id(UUID.randomUUID())
        .nome("Sala 401")
        .tipo("Sala de Aula")
        .quantidade(1)
    ...
    RecursoDTO recurso2 = RecursoDTO.builder()
        .id(UUID.randomUUID())
        .nome("Projetor")
        .tipo("Equipamento")
        .quantidade(5)
    ...
    List<RecursoDTO> recursos = Arrays.asList(recurso1, recurso2);
  ...
  @Test
  @DisplayName("GET /api/recursos/{id} - Deve retornar 200 e recurso quando ID válido")
  void deveRetornar200ERecursoQuandoIdValido() throws Exception {
    // Given
    UUID recursoId = UUID.randomUUID();
    RecursoDTO recurso = RecursoDTO.builder()
        .id(recursoId)
        .nome("Sala 401")
        .tipo("Sala de Aula")
        .quantidade(1)
    ...
    when(recursoService.findById(recursoId)).thenReturn(recurso);

    // When & Then
    mockMvc.perform(get("/api/recursos/" + recursoId)
        .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$.id").value(recursoId.toString()))
        .andExpect(jsonPath("$.nome").value("Sala 401"))
        .andExpect(jsonPath("$.tipo").value("Sala de Aula"))
        .andExpect(jsonPath("$.quantidade").value(1))
        .andExpect(jsonPath("$.capacidade").value(40))
  ...
  @Test
  @DisplayName("GET /api/recursos/{id} - Deve retornar 404 quando recurso não encontrado")
  void deveRetornar404QuandoRecursoNaoEncontrado() throws Exception {
    // Given
    UUID recursoId = UUID.randomUUID();
    when(recursoService.findById(recursoId))
        .thenThrow(new RuntimeException("Recurso não encontrado"));
  ...
  void deveRetornar404QuandoRecursoNaoEncontradoParaExclusao() throws Exception {
    // Given
    UUID recursoId = UUID.randomUUID();
    doThrow(new RuntimeException("Recurso não encontrado"))
